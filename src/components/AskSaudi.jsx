  ...
  return (
    <section id="ask" style={{ background: '#0B3A28', color: '#F7F3EA', padding: 'clamp(60px,9vh,110px) clamp(18px,4vw,44px)' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        <div style={{ fontFamily: mono, fontSize: 10.5, letterSpacing: '.2em', color: 'rgba(217,183,120,.8)', marginBottom: 16 }}>٠٧ — EXPERIMENTAL</div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') setAnswer(answerFor(q)) }}
          ...
          <button
            type="button"
            onClick={() => setAnswer(answerFor(q))}
            style={{ padding: '16px 28px', borderRadius: 999, border: 0, background: '#D9B778', color: '#06281B', fontFamily: 'inherit', fontWeight: 600, fontSize: 14, cursor: 'pointer', minHeight: 44 }}
          >
            اسأل
          </button>
        </div>
        ...
        {answer && (
          <div style={{ marginTop: 26, padding: 26, border: '1px solid rgba(247,243,234,.14)', borderRadius: 4, background: 'rgba(247,243,234,.04)', animation: 'sp-fade .5s ease both' }}>
            <p style={{ fontSize: 15, lineHeight: 1.9, margin: '0 0 18px', color: 'rgba(247,243,234,.88)' }}>{answer.text}</p>
