...
export default function Stories() {
  ...
  useEffect(() => {
    const load = () => fetchStories().then((s) => { setStories(s); setLoaded(true) }).catch(() => setLoaded(true))
    load()
    ...
    <section id="stories" style={{ background: '#F7F3EA', padding: 'clamp(70px,11vh,140px) clamp(18px,4vw,44px)' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        {stories.length > 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: 'clamp(40px,8vh,90px) 20px', border: '1px dashed rgba(20,18,14,.18)', borderRadius: 4 }}>
            <p style={{ fontFamily: alexandria, fontSize: 'clamp(19px,2.2vw,26px)', color: '#0B3A28', margin: '0 0 22px' }}>الأرشيف ينتظر أول حكاية… خلّها حكايتك.</p>
            <a href="#story" style={{ display: 'inline-flex', padding: '14px 30px', borderRadius: 999, background: '#0B3A28', color: '#F7F3EA', fontWeight: 600, fontSize: 15 }}>وش قصتك؟</a>
          </div>
        ) : null}
      </div>
    </section>
